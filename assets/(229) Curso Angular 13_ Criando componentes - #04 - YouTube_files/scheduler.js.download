...
function fa(){Y();var a=p("ytglobal.schedulerInstanceInstance_");a&&(a&&"function"==typeof a.dispose&&a.dispose(),q("ytglobal.schedulerInstanceInstance_",null))}
function Y(){L(X());for(var a in Q)Q.hasOwnProperty(a)&&delete Q[Number(a)]}
function ha(a,b,c){if(!c)return c=void 0===c,-I(X(),a,b,c);var d=window.setTimeout(function(){var f=I(X(),a,b);Q[d]=f},c);
return d}
function ia(a){var b=X();G(b,a)}
function ja(a){var b=X();if(0>a)delete b.i[-a];else{var c=Q[a];c?(delete b.i[c],delete Q[a]):window.clearTimeout(a)}}
function Z(a){var b=p("ytcsi.tick");b&&b(a)}
function ka(){Z("jse");la()}
function la(){window.clearTimeout(S);X().start()}
function ma(){var a=X();K(a);a.C=!0;window.clearTimeout(S);S=window.setTimeout(ka,R)}
function na(){window.clearTimeout(T);T=window.setTimeout(function(){Z("jset");oa(0)},R)}
...
;p("yt.scheduler.initialized")||(q("yt.scheduler.instance.dispose",fa),q("yt.scheduler.instance.addJob",ha),q("yt.scheduler.instance.addImmediateJob",ia),q("yt.scheduler.instance.cancelJob",ja),q("yt.scheduler.instance.cancelAllJobs",Y),q("yt.scheduler.instance.start",la),q("yt.scheduler.instance.pause",ma),q("yt.scheduler.instance.setPriorityThreshold",oa),q("yt.scheduler.instance.enablePriorityThreshold",pa),q("yt.scheduler.instance.clearPriorityThreshold",qa),q("yt.scheduler.initialized",!0));}).call(this);
